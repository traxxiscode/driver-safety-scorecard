/* =========================================================
   Baseline Driver Safety Scorecard — safety.js
   Storage: Cloud Firestore (replaces localStorage)
   All rule config, weights, and schedule data are persisted
   to Firestore so the AWS EC2 backend can read them.
   =========================================================
   FIRESTORE SETUP (replace with your project config):
     - The geotab database name is used as the Firestore
       document ID so multiple databases are isolated.
     - Collection structure:
         safety_config/{geotabDatabase}/ruleConfig    → object
         safety_config/{geotabDatabase}/ruleWeights   → object
         safety_config/{geotabDatabase}/schedule      → object
         safety_config/{geotabDatabase}/preferences   → object
   ========================================================= */
...
var FS = (function () {
  ...
  function load(field, cb) {
    if (!_ready) {
      _queue.push(function () { load(field, cb); });
      return;
    }
    docRef(field).get()
      .then(function (snap) {
        cb(null, snap.exists ? snap.data() : null);
      })
      .catch(function (e) {
        console.error('[SafetyScorecard] Firestore load error:', e);
        cb(e, null);
      });
  }

  function remove(field, cb) {
    if (!_ready) { if (typeof cb === 'function') cb(null); return; }
    docRef(field).delete()
...
var safetyDash = (function () {
  ...
  /* ── Firestore: load config on startup ── */
  ...
  /* After a rule toggle — auto-save ruleConfig to Firestore */
  function onRuleToggled() {
    equalizeWeightsData();
    FS.save('ruleConfig', _ruleConfig);
  ...
  /* ── Schedule: Firestore persistence ── */
  function loadSchedule(cb) {
    FS.load('schedule', function (err, data) {
  ...
  /* ── Public API ── */
  return {
    /* Called by Geotab addin lifecycle */
    init: function (api, geotabState) {
      _api = api;
      setDateRange();
      ...
      /* Derive the Geotab database name for Firestore document isolation */
      var dbName = (geotabState && geotabState.server) ? geotabState.server.replace(/[^a-zA-Z0-9_-]/g, '_') : 'default';

      /*
       * ── CONFIGURE FIRESTORE ──────────────────────────────
       * Replace the firebaseConfig object below with your
       * actual Firebase project settings.
       * ────────────────────────────────────────────────────
       */
      FS.configure({
      ...
      FS.load('preferences', function (err, prefs) {
    ...
    toggleTheme: function () {
      ...
      FS.save('preferences', { theme: _isLight ? 'light' : 'dark' });
    },
    ...
    resetWeights: function () {
      FS.remove('ruleWeights', function () {
        _ruleWeights = {};
        equalizeWeightsData();
        renderWeightsPanel();
        if (_rawData.length) rebuildRows();
    ...
    toggleGroup: function (gid) {
      var idx = _selectedGroups.indexOf(gid);
      if (idx === -1) _selectedGroups.push(gid); else _selectedGroups.splice(idx, 1);
      renderGroupDropdown(); renderTable(getDisplayRows()); renderKPIs(getDisplayRows());
    },
    ...
    disableSchedule: function () {
      _schedEnabled = false;
      saveScheduleData(function () { updateSchedStatus(); toast('Schedule disabled', '#64748b'); });
    },
...
function updateLogos(isLight) {
  ...
  document.querySelectorAll('.hdr-logo').forEach(function (img) {
